package com.foodkeeper.core.ui.util

import androidx.compose.ui.graphics.Color

object AppColors {

    val main = Color(0xFFFF7F1E)       // 메인 컬러 - 주황색
    val point = Color(0xFFFECE00)      // 포인트 컬러 - 노란색
    val black = Color(0xFF2F2F2F)       // 메인 텍스트 - 검정
    val light2Gray = Color(0xFFA2A2A2)  // 회색 텍스트 컬러
    val white = Color(0xFFFBFBFB)      // 흰색
    val light3Gray = Color(0xFFBBBBBB)  // 밝은 회색3
    val light4Gray = Color(0xFFD7D7D7)  // 밝은 회색4
    val light5Gray = Color(0xFFDEDEDE)  // 밝은 회색5
    val light6Gray= Color(0xFFF5F5F5)  // 밝은 회색7

    val dartGray = Color(0xFF8C8C8C)  // 밝은 회색
}